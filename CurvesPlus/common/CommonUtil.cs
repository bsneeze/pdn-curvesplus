...
namespace pyrochild.effects.common
{
    ...
    {
        public static int IntDiv(int x, int z)
        {
            //Integer Division by byte

            //Short enough to be in-lined by JIT compiler.
            //Quicker than x86 integer division

            int i = z * 3;
            return (int)(((x * masTable[i]) + masTable[i + 1]) >> (int)masTable[i + 2]);
        }
        ...
        public static RectangleF SquareFromCenter(float x, float y, float size)
        {
            return new RectangleF(x - size / 2, y - size / 2, size, size);
        }
    }
}